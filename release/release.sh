#!/bin/bash
#
# release.sh — build, notarise, Sparkle-sign and publish a new Oatly release.
#
#   Usage:  ./release/release.sh <marketing-version> <build-number>
#   e.g.    ./release/release.sh 1.1 2
#
# What it does:
...
#
# Ported from OhDelhi's release.sh (see [[2026-06-25 07-37 OhDelhi
# deterministic Amazon parser + Sparkle auto-update]]) — same recipe, only
# the config block below differs.
#
...
#   - A shared Xcode scheme named "Oatly" exists for the Mac target (check
#     Product ▸ Scheme ▸ Manage Schemes — the project's scheme bookkeeping
#     is stale from the OT→Oatly rename, so this isn't guaranteed to exist)
...
APP_NAME="Oatly"
SCHEME="Oatly"
PROJECT="Oatly.xcodeproj"
GITHUB_REPO="dt111270/Oatly"              # your GitHub slug
NOTARY_PROFILE="oatly-notary"             # `xcrun notarytool store-credentials` profile name
MIN_MACOS="26.3"                          # minimum supported macOS (match deployment target)
...
echo "==> Done. Oatly $VERSION published; the Mac will pick it up on its next check."
